...
ANSWER: The realtionship between 'Employee' and 'Territory" tables is one-to-many because one employee may have more than one territory (while a territory can only have one employee assigned to it).

- What is a situation where a document store (like MongoDB) is appropriate, and what is a situation where it is not appropriate?
Answer: A document store (like MongoDB) is appropriate when dealing with Big Data (large volume, large velocity of data creation, etc.) where the relational nature between the documents is not as important and speed and scalability are imperative. MongoDB is also appropriate where there are unrelated, indeterminate or evolving data requirements i.e Unstructured or semi-structured data because there is no need for a schema to be defined before the data is loaded. MongoDB is not appropriate where the relational nature of the data is important and there is a need for ACID (Atomicity, Consistency, Isolation, and Durability) transactions.

- What is "NewSQL", and what is it trying to achieve?
Answer: "NewSQL" refers to a set of new database management tools that combine some of the features of traditional relational SQL databases and the non-relational NoSQL databases. NewSQL is trying to achieve the horizontal scalabilty and high availability of NoSQL while still guaranteeing the ACID compliance of relational SQL databases.
